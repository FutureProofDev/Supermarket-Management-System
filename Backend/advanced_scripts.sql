...
-- (Sample data currently has no products at/below reorder level — the
-- query is correct and will populate as stock is sold down; verified
-- separately against inventory levels during testing.)
-- ---------------------------------------------------------------------
SELECT
    p.product_id,
    p.name,
...
-- ---------------------------------------------------------------------
-- Q7. Revenue contribution % by category (window function: SUM() OVER())
-- Business use: category mix analysis for buying/merchandising decisions.
-- ---------------------------------------------------------------------
SELECT
    c.name AS category,
    SUM(si.line_total) AS category_revenue,
    ROUND(100 * SUM(si.line_total) / SUM(SUM(si.line_total)) OVER (), 2) AS pct_of_total_revenue
FROM sale_item si
...
FROM (
    SELECT DATE(sale_date) AS sale_date_only, SUM(total_amount) AS daily_revenue
    FROM sale
...
-- near-expiry discount (see app/routers/sales.py) and physical markdown
-- tagging on the shop floor.
-- ---------------------------------------------------------------------
SELECT
    p.product_id,
    p.name,
...
WHERE p.expiry_date IS NOT NULL
  AND DATEDIFF(p.expiry_date, CURDATE()) <= 45
...
-- candidates for expanded shelf space or featured promotion.
-- Uses two chained CTEs rather than a correlated subquery through a
-- derived table (which MySQL/MariaDB does not allow to see the outer
-- query's columns).
-- ---------------------------------------------------------------------
...
SELECT
    pr.product_id, pr.name, pr.category_id, pr.product_revenue,
...
-- Business use: sanity-check whether more experienced roles (Head
-- Cashier) ring up larger baskets than Cashiers, informing scheduling.
-- ---------------------------------------------------------------------
SELECT
    e.role,
